...
def main(sub_name_dicts, cv_params, aug=True, intermediate=False, save_cp=False, save_wei=True,
    ...
    for fld in range(res_fold, fold):
        ...
        for sub in range(len(test_sub_dicts)):
            ...
            probs_axial = truenet_tumseg_data_postprocessing.resize_testdata_to_original_size(probs_axial, test_sub_dict,
                                                                                     plane='axial')
            probs_combined.append(probs_axial)

            if intermediate:
                save_path = os.path.join(output_dir, 'Predicted_probmap_truenet_' + basename + '_axial.nii.gz')
                preds_axial = truenet_tumseg_data_postprocessing.get_final_testdata_3dvolumes(probs_axial, test_sub_dict)
                if verbose:
            ...
            probs_sagittal = truenet_tumseg_data_postprocessing.resize_testdata_to_original_size(probs_sagittal, test_sub_dict,
                                                                                        plane='sagittal')
            ...
            if intermediate:
                ...
                preds_sagittal = truenet_tumseg_data_postprocessing.get_final_testdata_3dvolumes(probs_sagittal, test_sub_dict)
                if verbose:
            ...
            probs_coronal = truenet_tumseg_data_postprocessing.resize_testdata_to_original_size(probs_coronal, test_sub_dict,
                                                                                       plane='coronal')
            probs_combined.append(probs_coronal)

            if intermediate:
                save_path = os.path.join(output_dir, 'Predicted_probmap_truenet_' + basename + '_coronal.nii.gz')
                preds_coronal = truenet_tumseg_data_postprocessing.get_final_testdata_3dvolumes(probs_coronal, test_sub_dict)
                if verbose:
            ...
            pred_mean = truenet_tumseg_data_postprocessing.get_final_testdata_3dvolumes(prob_mean, test_sub_dict)

            pred_output = np.argmax(prob_mean, axis=3)
            pred_output = truenet_tumseg_data_postprocessing.get_final_testdata_3dvolumes(pred_output, test_sub_dict)

            if postproc is False:
                save_path = os.path.join(output_dir, 'Predicted_probmap_truenet_' + basename + '_befpp.nii.gz')
                if verbose:
                ...
                if intermediate:
                    save_path = os.path.join(output_dir, 'Predicted_probmap_truenet_' + basename + '_befcorr_wotc.nii.gz')
                    if verbose:
                ...
                pred_final = truenet_tumseg_utils.post_processing_without_tc(pred_output)
                ...
                probs_tc = truenet_tumseg_data_postprocessing.resize_testdata_to_original_size(probs_tc, test_sub_dict,
                                                                                      plane='tc')
                preds_tc = truenet_tumseg_data_postprocessing.get_final_testdata_3dvolumes(probs_tc, test_sub_dict)
                pred_final = truenet_tumseg_utils.post_processing_including_tc(pred_output, preds_tc)

                if intermediate:
                    save_path = os.path.join(output_dir, 'Predicted_probmap_truenet_' + basename + '_befcorr_tc.nii.gz')
                    if verbose:
                        print('Saving the intermediate Tumour core prediction ...', flush=True)
